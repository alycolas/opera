	...
	if(prefs.openmini){
		...
		document.addEventListener('click',function(e){
			...
			if(a){
				...
				if(t_id){
					e.preventDefault();
					var xwindow=window.open("http://v.youku.com/v_showMini/"+t_id+"_ft_0.html","minWin_"+Math.random(),"height=548,width=900,resizable=yes,toolbar=no,menubar=no,scrollbars=no,location=no,status=yes,alwaysRaised=yes,");
			...
			(function(){
				...
				function V_download(e){
					if(!i_downbox){
						...
						iframe.src=i_src;
						//window.open('http://www.flvcd.com/parse.php?kw='+url+(prefs.HDD? '&format=high' : ''),'N_downvideo','width=300,height=100');
