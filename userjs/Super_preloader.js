...
*/
        ...
        function autopager(SSS,floatWO){
            ...
            var paged=0;
            uncheck=false;
            var currentpage,secondpage,firstpage=getFullHref(window.location.href);
            var currenthash,secondhash,firsthash=parseFloat(firstpage.replace(/\D/g, ""));
            function insertedIntoDoc(){
                ...
                //循环检查初始化getFullHref(currentpage).replace(/\D/g, "")
                currentpage=getFullHref(nextlink);
                currenthash=parseFloat(currentpage.replace(/\D/g, ""));
                if(!SSS.uncheck){
                    if(paged==0){
                        secondpage=getFullHref(nextlink);                        //初始化
                        secondhash=parseFloat(secondpage.replace(/\D/g, ""));
                    };      
                    //比较第一页或者第二页
                    if(firsthash>=currenthash&&firstpage.length-currentpage.length<=5 || secondhash>=currenthash&&paged>1){//修正第一页地址
                        removeL();
                        return;
                    };
                };
        ...
        for(i=0;i<ii;i++){
            ...
            if(Rurl.test(url)){
                ...
                if(SIIA){
                    ...
                    SSS.uncheck=(SIIA.uncheck===undefined)? SIIAD.uncheck : SIIA.uncheck;
