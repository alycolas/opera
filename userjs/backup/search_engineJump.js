...
(function(){
	//时间时间啊..
	//var T_time1=new  Date();
	//iframe上放弃加载.
	if(self!=window.parent)return;

	//几个开关..
	var prefs={
	/*
	**同一个页面上 2 种模式只能加载一种..高级规则模式.. 优先级最高;
	**工具栏模式..是在没有高级规则的页面上加载的;
	*/
	//------------------全局开关设置-----------------------------//
		transition:true										,//下拉列表..开启动画拉伸和渐入(需浏览器支持CSS3 transition)
				timF:'0.5,0.8,0.8,1.0'				,//动画效果(chrome不支持 大于1的数..否则可以实现反弹效果,opera,FX都支持哦) 如何设置参见:http://www.w3.org/TR/css3-transitions/#transition-duration
				duration:0.3									,//动画持续时间:单位(毫秒)
		favicon:true											,//加载站点图标
	//------------------高级规则模式设置-------------------------//
		E_advanced:true										,//(高级规则模式)在有高级规则的网页上加载
				separator:' '									,//用一个标点来分隔每个搜索引擎.默认的一个半角空格,可以换成全角空格等来获取更大的间距
				D_lists:false								,//禁用所有的下拉列表
				newpage:true								,//默认在新标签打开引擎链接
	//------------------工具栏模式设置--------------------------//
		E_TB:false												,//(工具栏模式)在没有规则的网站.在顶部显示所有搜索下拉列表;
				fixed:false									,//永久的固定..(opera对fixed元素处理,会导致opera的平滑滚动很卡..opera的硬伤.)
						Tposition:1								,//(1:左上角,2:右上角)永久的固定..(opera对fixed元素处理,会导致opera的平滑滚动很卡..opera的硬伤.)
				D_open:0											,//在输入完成后,敲回车默认用 0:Google搜索, 1：百度搜索
				newpage2:true									,//默认新标签打开链接
	};
	...
	var g_sls='\
		...
		div.Nexdiv{\
			...
			-moz-box-shadow:2px 2px 5px rgba(0,0,0,0.6)!important;\
			-webkit-box-shadow:2px 2px 5px rgba(0,0,0,0.6)!important;\
			...
			-moz-border-radius:8px!important;'+(prefs.transition? '\
			-o-transition:height 0.3s cubic-bezier('+prefs.timF+'),opacity '+prefs.duration+'s ease-in-out;\
			-moz-transition:height 0.3s cubic-bezier('+prefs.timF+'),opacity '+prefs.duration+'s ease-in-out;\
			-webkit-transition:height 0.3s cubic-bezier('+prefs.timF+'),opacity '+prefs.duration+'s ease-in-out;\
			':'')+'\
		}\
		/*限制高度和字体大小*/\
		div#slist,div.Nexdiv{\
		...
		/*已访问的a元素*/\
		div#slist a:visited,div.Nexdiv a:visited{\
		...
		div.Nexdiv a:hover{\
			...
			background-color:#2E7CF0!important;\
			box-shadow:inset 0 13px 10px rgba(255,255,255,0.4)!important;\
			-moz-box-shadow:inset 0 13px 10px rgba(255,255,255,0.4)!important;\
			-webkit-box-shadow:inset 0 13px 10px rgba(255,255,255,0.4)!important;\
		}\
		div.Nexdiv a:first-child:hover{\
			border-radius:8px 8px 0 0!important;\
			-moz-border-radius:8px 8px 0 0!important;\
		}\
		div.Nexdiv a:last-child:hover{\
			border-radius:0 0 8px 8px!important;\
			-moz-border-radius:0 0 8px 8px!important;\
		}\
		/*图标样式*/\
		div#slist img,div.Nexdiv img{\
	...
	var isites=[
		{sitename:"google视频",
			site_regexp:/^https?:\/\/\w{2,10}\.google(?:\.\D{1,3}){1,2}\/search\?.+&tbs=vid/i,
			slist:"video",
			div_style:"\
			...
			keyword:function(){
				...
				var i,
							ii,
							qs_x;
				for(i=0,ii=qs.length;i<ii;i++){
					qs_x=qs[i];
					if(qs_x.form.id=='tsf')return qs_x.value;
				};
			},
			position:'//div[@id="subform_ctrl"]/following-sibling::*[1]',
		},
		//网页搜索/////////////第一个可以当模板看///////////////////////////////////////////////////////////////////////////////
		{sitename:"google网页搜索",																								//你要加载的网站的名字(方便自己查找)
			enable:true,																														//是否启用(除非此项的false,此项为true或者不写此项都是启用)
			site_regexp:/^https?:\/\/\w{2,10}\.google(?:\.\D{1,3}){1,2}\/search\?/i,//在哪个网站上加载.
			site_example:"http://www.google.*/search?",															//例子(方便了自己读,此项可以没有)
			slist:"web",																														//加载哪个类型的列表??? web(网页),music(音乐),video(视频),pic(图片),dl(下载),spp(购物),tra(翻译),留空(所有的列表全部下拉.)
			loaded:false,																														//在load事件后加载.chrome上无效.不解释.
			div_style:'\
				padding-left:10px;\
			...
			},																														//如何获取这个网站搜索关键字?(请返回没有额外编码过的关键字)
			position:['//*[@id="ssb"]','//div[@id="subform_ctrl"]/following-sibling::*[1]'],								//插入这个网站的哪个位置呢? 请使用xpath匹配,可以使用无数个位置候选(越靠前,优先越高)
			insert_method:1																													//以什么方式插入呢? 1 为插到position之前. 2 为附加到position里面(省略此项时..默认为 1)
		},
		{sitename:"baidu网页搜索",
			site_regexp:/^https?:\/\/www\.baidu\.com\/(?:s|baidu)/i,
			site_example:"http://www.baidu.com/s?,http://www.baidu.com/baidu?",
			slist:"web",
			div_style:'\
				padding-left:5px;\
				margin-left:0px;\
		 ...
		 {sitename:"豆瓣",
         site_regexp:/^http:\/\/\w+\.douban\.com\/subject\/\d+/i,
         site_example:"http://movie.douban.com/subject/3541415/",
         slist:"web",
         div_style:"\
            margin-top:5px;\
       ...
       {sitename:"豆瓣搜索",
         site_regexp:/^http:\/\/\w+\.douban\.com\/subject_search\?/i,
         site_example:"http://movie.douban.com/subject/3541415/",
         slist:"web",
         div_style:"\
            margin-top:5px;\
         ...
         ",
         keyword:function(){
            return document.getElementsByName('search_text')[0].value;
         },
         position:'//*[@id="header"]',
         insert_method:2,
      },      
		{sitename:"baidu百科",
			site_regexp:/^https?:\/\/baike\.baidu\.com\/(?:view|searchword)/i,
			site_example:"http://baike.baidu.com/view/10019.htm",
			slist:"web",
			div_style:'\
				padding-left:5px;\
				margin-left:0px;\
			...
			position:'//div[@id="path"]',
		},
		{sitename:"必应网页搜索",
			site_regexp:/^https?:\/\/.*\.bing\.com\/search/i,
			site_example:"http://cn.bing.com/search?,http://com.bing.com/search?",
			slist:"web",
			div_style:'\
				margin-left:2px;\
			...
			position:'//div[@id="sw_canvas"]',
		},
		{sitename:"有道网页搜索",
			site_regexp:/^https?:\/\/www\.youdao\.com\/search/i,
			site_example:"www.youdao.com/search",
			slist:"web",
			div_style:'\
				position:relative;\
				top:0;\
		...
		{sitename:"搜狗网页搜索",
			site_regexp:/^https?:\/\/www\.sogou\.com\/(?:web|sogou)/i,
			site_example:"www.sogou.com/web",
			slist:"web",
			div_style:"\
				padding-left:10px;\
			...
			position:'//*[@class="resultbar"]',
		},
		{sitename:"搜搜网页搜索",
			site_regexp:/^https?:\/\/www\.soso\.com\/q/i,
			site_example:"www.soso.com/q",
			slist:"web",
			div_style:"\
				text-align:left;\
				padding-left:10px;\
			...
			",
			keyword:function(){
				return document.getElementById('smart_input').value
			},
			position:'//*[@id="s_main"]',
		},
		{sitename:"雅虎网页搜索",
			site_regexp:/^https?:\/\/.*\.yahoo\.com\/s.*web/i,
			site_example:"one.cn.yahoo.com/s",
			slist:"web",
			div_style:"\
				border-top:1px solid #D4E9F7;\
			",
		...
		//音乐搜索/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		{sitename:"谷歌音乐",
			site_regexp:/^https?:\/\/www\.google\.cn\/music\/search/i,
			site_example:"www.google.cn/music/search",
			slist:"music",
			div_style:"\
				text-align:left!important;\
				border-bottom:1px solid #D4E9F7;\
			",
			keyword:function(){
				return document.getElementById('music_search_box').value
			},
			position:['//*[@id="music_header"]','//div[@id="subform_ctrl"]'],
		},
		{sitename:"百度MP3",
			site_regexp:/^https?:\/\/mp3\.baidu\.com\/m/i,
			site_example:"mp3.baidu.com/m",
			slist:"music",
			div_style:"\
				padding-left:10px;\
			...
			",
			keyword:function(){
				return document.getElementById('ww').value
			},
			position:'/html/body/table[2]',
		},
		{sitename:"搜搜音乐",
			site_regexp:/^https?:\/\/cgi\.music\.soso\.com/i,
			site_example:"cgi.music.soso.com",
			slist:"music",
			div_style:"\
				margin-top:2px;\
			...
			position:'//*[@id="search_result"]',
		},
		{sitename:"有道音乐",
			site_regexp:/^https?:\/\/mp3\.youdao\.com\/search/i,
			site_example:"mp3.youdao.com/search",
			slist:"music",
			div_style:"\
				padding-left:10px;\
				margin-top:2px;\
		...
		{sitename:"搜狗音乐",
			site_regexp:/^https?:\/\/mp3\.sogou\.com\/music\.so/i,
			site_example:"mp3.sogou.com/music.so",
			slist:"music",
			div_style:"\
				text-align:left;\
				padding-left:10px;\
			...
			position:'//*[@id="songlist"]',
		},
		{sitename:"狗狗音乐",
			site_regexp:/^https?:\/\/mp3\.gougou\.com\/search/i,
			site_example:"mp3.gougou.com/search",
			slist:"music",
			div_style:"\
				padding-left:10px;\
				margin-top:2px;\
			...
			",
			keyword:function(){
				return document.getElementById('s1').value
			},
			position:'/html/body/div[2]',
		},
		{sitename:"雅虎音乐",
			site_regexp:/^https?:\/\/.*\.yahoo\.com\/s.*music/i,
			site_example:"search.cn.yahoo.com/s*v=musi*",
			slist:"music",
			div_style:"\
				padding-left:10px;\
		...
		{sitename:"一听音乐",
			site_regexp:/^https?:\/\/so\.1ting\.com\/all\.do/i,
			site_example:"so.1ting.com/all.do",
			slist:"music",
			div_style:"\
				padding-left:10px;\
		...
		{sitename:"songtaste",
			site_regexp:/^https?:\/\/www\.songtaste\.com\/search/i,
			site_example:"http://www.songtaste.com/search",
			slist:"music",
			div_style:"\
			...
			",
			keyword:function(){
				return document.getElementById('sb').value
			},
			position:'//*[@id="subnav"]',
		},
		{sitename:"xiami",
			site_regexp:/^https?:\/\/www\.xiami\.com\/search/i,
			site_example:"http://www.xiami.com/search",
			slist:"music",
			div_style:"\
			...
			position:'//div[@class="search_result"]',
		},
		{sitename:"猫扑爱听",
			site_regexp:/^https?:\/\/m\.mop\.com\/search/i,
			site_example:"http://m.mop.com/search/",
			slist:"music",
			div_style:"\
				border-top:1px solid #E7E7DF;\
			",
			...
			position:'//div[@class="recommec_wrap"]',
		},
		{sitename:"巨鲸",
			site_regexp:/^https?:\/\/www\.top100\.cn\/search/i,
			site_example:"http://www.top100.cn/search/",
			slist:"music",
			div_style:"\
			...
			position:'//div[@class="topbar"]',
		},
		//图片搜索//////////////////////////////////////////////////////////////////////////////////////////////////////
		{sitename:"百度图片",
			site_regexp:/^https?:\/\/image\.baidu\.com\/i/i,
			site_example:"image.baidu.com/i",
			slist:"pic",
			div_style:"\
				padding-left:10px;\
				border-top:1px solid #CDEAF6;\
			...
			position:'//*[@id="filterBanner"]',
		},
		{sitename:"谷歌图片",
			site_regexp:/^https?:\/\/(?:images|www)\.google\..*?\/images/i,
			site_example:"images.google.cn/images",
			slist:"pic",
			div_style:"\
				padding-left:10px;\
				border-top:1px solid #CDEAF6;\
			...
			position:['//*[@id="ssb"]','//div[@id="subform_ctrl"]'],
		},
		{sitename:"必应图片",
			site_regexp:/^https?:\/\/.*\.bing\.com\/images\/search/i,
		...
		{sitename:"搜搜图片",
			site_regexp:/^https?:\/\/image\.soso\.com\/image/i,
			site_example:"image.soso.com/image",
			slist:"pic",
			div_style:"\
				text-align:left;\
				padding-left:10px;\
		...
		{sitename:"搜狗图片",
			site_regexp:/^https?:\/\/pic\.sogou\.com\//i,
			site_example:"pic.sogou.com/pics",
			slist:"pic",
			div_style:"\
				padding-left:10px;\
			...
			position:'//*[@id="resultbar"]',
		},
		{sitename:"雅虎图片",
			site_regexp:/^https?:\/\/.*\.yahoo\.com\/s.*image/i,
			site_example:"cn.yahoo.com/s?*v=imag",
			slist:"pic",
			div_style:"\
				padding-left:10px;\
				border-top:1px solid #EBF1FF;\
		...
		{sitename:"有道图片",
			site_regexp:/^https?:\/\/image\.youdao\.com\/search/i,
			site_example:"image.youdao.com/search",
			slist:"pic",
			div_style:"\
				padding-left:10px;\
				border-top:1px solid #EBF1FF;\
		...
		{sitename:"flickr",
			site_regexp:/^http:\/\/www\.flickr\.com\/search\/\?/i,
			site_example:"http://www.flickr.com/search/?",
			slist:"pic",
			keyword:function(){
				return document.getElementById('standard_q').value
			},
			position:'//*[@id="SearchForm"]',
			insert_method:2
		},
		{sitename:"picsearch",
			site_regexp:/^http:\/\/cn\.picsearch\.com\/index\.cgi/i,
			site_example:"http://cn.picsearch.com/index.cgi",
			slist:"pic",
			div_style:"\
				margin-left:20px;\
			",
			...
			position:'//div[@id="parentContainer"]',
		},
		//视频搜索//////////////////////////////////////////////////////////////////////////////////////////////////////
		{sitename:"优酷",
			site_regexp:/^https?:\/\/www\.soku\.com\/search/i,
			site_example:"so.youku.com/search",
			slist:"video",
			div_style:"\
				text-align:center;\
				border-top:1px solid #DDF6FC;\
			",
			keyword:function(){
				return document.getElementById('headq').value
			},
			position:['//div[contains(@class,"viewTab")]'],
		},
		{sitename:"土豆",
			site_regexp:/^https?:\/\/so\.tudou\.com\/.*search/i,
			site_example:"so.tudou.com/isearch",
			slist:"video",
			div_style:"\
				width:970px;\
				border-bottom:1px solid #F36A0A;\
			",
			keyword:function(){
				return document.getElementsByName('kw')[0].value
			},
			position:'/html/body/div/div[2]',
		},
		{sitename:"百度视频",
			site_regexp:/^https?:\/\/video\.baidu\.com\/v/i,
			site_example:"video.baidu.com/v",
			slist:"video",
			div_style:"\
		...
		{sitename:"必应视频",
			site_regexp:/^https?:\/\/.*\.bing\.com\/videos\/search/i,
			site_example:"bing.com/videos/search",
			slist:"video",
			div_style:"\
				padding-left:5px;\
				border-bottom:1px solid #E6E6E6;\
		...
		{sitename:"有道视频",
			site_regexp:/^https?:\/\/video\.youdao\.com\/search/i,
			site_example:"video.youdao.com/search",
			slist:"video",
			div_style:"padding-left:5px;border-bottom:1px solid #D8D8FF;",
			keyword:function(){return document.getElementById('query').value},
			position:'//*[@id="w"]',
		},
		{sitename:"56视频",
			site_regexp:/^https?:\/\/so\.56\.com\/\w+\/.+\/\?/i,
			site_example:"so.56.com/index",
			slist:"video",
			div_style:"\
				text-align:center;\
			...
			position:'//body/div[contains(@class,"wrap")]',
		},
		{sitename:"6房间",
			site_regexp:/^https?:\/\/6\.cn\/search\.php/i,
			site_example:"http://6.cn/search.php?k=",
			slist:"video",
			div_style:"\
				text-align:center;\
			...
			",
			keyword:function(){
				return document.getElementsByName('k')[0].value;
			},
			position:'//div[@id="content"]',
		},
		{sitename:"ku6",
			site_regexp:/^http?:\/\/so\.ku6\.com\/\w\/q/i,
			site_example:"http://so.ku6.com/m/q火狐",
			slist:"video",
			div_style:"\
				text-align:center;\
				border-bottom:1px solid #298C32;\
				border-top:1px solid #298C32;\
			",
			keyword:function(){
				return document.getElementById('index_input').value;
			},
			position:'//div[@class="mainNav"]',
		},
		//下载//////////////////////////////////////////////////////////////////////////////////////
		{sitename:"绿色下载站",
			site_regexp:/^https?:\/\/search\.greendown\.cn\/search/i,
			site_example:"http://search.greendown.cn/search",
			slist:"dl",
			keyword:function(){
				var li_right=document.getElementsByClassName('right')[0];
				return li_right.childNodes[1].firstChild.nodeValue;
			},
			position:'//*[@class="nav-breadcrumb"]',
		},
		{sitename:"绿盟",
			site_regexp:/^https?:\/\/www\.xdowns\.com\/(?:tag|soft).+/i,
			site_example:"http://www.xdowns.com/tag/",
			slist:"dl",
			keyword:function(){
				return document.getElementById('Addresslinecenter').textContent.match(/下载中心 -> (.*) - xDowns Tag/i)[1];
			},
			position:'//*[@id="searchpageResult"]',
		},
		{sitename:"射手字幕",
			site_regexp:/^https?:\/\/shooter\.cn\/search/i,
			site_example:"http://shooter.cn/search",
			slist:"dl",
			div_style:"\
			...
			",
			keyword:function(){
				/search\/Sub:(.*?)\//i.test(location.href);
				return(decodeURIComponent(RegExp.$1));
			},
			position:'//*[@class="statline"]',
		},
		{sitename:"verycd搜索",
			site_regexp:/^https?:\/\/www\.verycd\.com\/search\//i,
			site_example:"http://www.verycd.com/search/folders",
			slist:"dl",
			div_style:"\
				margin-buttom:2px;\
			...
			",
			keyword:function(){
				return document.getElementById('top-search-input').value
			},
			position:'//*[@id="page-body"]',
		},
		{sitename:"狗狗搜索",
			site_regexp:/^https?:\/\/(?:www|web)\.gougou\.com\/search/i,
			site_example:"http://www.gougou.com/search",
			slist:"dl",
			div_style:"\
				margin-left:0px;\
				border-top:1px solid #B6CAE3;\
		...
		//购物/////////////////////////////////////////////////////////////////////////////////////////
		{sitename:"淘宝搜索",
			site_regexp:/^https?:\/\/search\.taobao\.com\/search/i,
			site_example:"http://search.taobao.com/search",
			slist:"spp",
			div_style:"\
		...
		//翻译/////////////////////////////////////////////////////////////////////////////////////////
		{sitename:"有道词典",
			site_regexp:/^https?:\/\/dict\.youdao\.com\/search/i,
			site_example:"http://dict.youdao.com/search",
			slist:"tra",
			div_style:"\
				border-bottom:1px solid #D4E9F7;\
				border-top:1px solid #D4E9F7;\
		...
		{sitename:"沪江",
			site_regexp:/^https?:\/\/dict\.hjenglish\.com\/app\/\w+\//i,
			site_example:"http://dict.hjenglish.com/app/jp/jc/おぺら",
			slist:"tra",
			div_style:"\
				border-top:1px solid #DFF4A6;\
			",
			keyword:function(){
				return document.getElementById('w').value
			},
			position:'//*[@id="container"]',
		},
	];

	//你自己定义的需要将引擎跳转插入网站(优先级比JS自带的高)..更新本JS的时候..直接将这个数组复制到新JS就好了..更新也方便..
	var cus_isites=[
	];
	...
	var base64_icons={
		...
		,soso:'data:image/png;base64,'
		...
		,youa:'data:image/png;base64,'
		...
		//,wmzhe:'data:image/png;base64,'
	};

	//站点在线请求图标
	var ol_icons={
	    douban:'http://img3.douban.com/favicon.ico'
		...
		,youa:'http://youa.com/favicon.ico'
	...
	var icons={
		...
		,youa:base64_icons.youa || ol_icons.youa
	...
	//如何添加搜索引擎:
	//比如:
	//wo[0]=['Google','http://www.google.com/search?q=%s&sourceid=opera&ie=utf-8&oe=utf-8','http://www.google.cn/favicon.ico','gbk']
	//wo[排序(从小到大排列,小于0不显示相应引擎)]['显示在网页上的名字','关键字变量用%s代替','站点的图标(可以没有)',是否不接受UTF-8编码(不接受的话..填相应的编码..目前支持gbk或gb2312)]
	//网页搜索列表
	var wo=[];
	wo[0]=['Google','http://www.google.com/search?q=%s&sourceid=opera&ie=utf-8&oe=utf-8',icons.google]
	wo[1]=['百度','http://www.baidu.com/s?wd=%s&tn=cnopera&ie=utf-8',icons.baidu]
	wo[2]=['豆瓣','http://movie.douban.com/subject_search?search_text=%s&cat=1002','http://img3.douban.com/favicon.ico']
	wo[3]=['必应','http://cn.bing.com/search?q=%s&pc=OPER',icons.bing]
	wo[4]=['有道','http://www.youdao.com/search?q=%s&ue=utf8',icons.youdao]
	wo[5]=['雅虎','http://one.cn.yahoo.com/s?p=%s&v=web&fr=opera2',icons.yahoo]
	wo[6]=['搜搜','http://www.soso.com/q?&w=%s&ie=utf-8&oe=utf-8',icons.soso]
	wo[7]=['搜狗','http://www.sogou.com/web?query=%s',icons.sogou]
	wo[8]=['维基(ZH)','http://zh.wikipedia.org/wiki/Special:Search?search=%s',icons.wiki]
	wo[-8]=['维基(EN)','http://en.wikipedia.org/wiki/Special:Search?search=%s',icons.wiki]
	wo[9]=['百度百科','http://baike.baidu.com/searchword/?word=%s&pic=1&sug=1&tn=cnopera&ie=utf-8&oe=utf-8',icons.baidu,'gbk']
	wo[-10]=['ishare','http://ishare.iask.sina.com.cn/search.php?key=%s&from=index&format=',icons.iask,'gbk']
	
       

	//视频搜索列表
	var vo=[];
	vo[1]=['优酷','http://www.soku.com/search_video/q_%s',icons.youku]
	vo[2]=['土豆','http://so.tudou.com/isearch/%s',icons.tudou]
	vo[3]=['56','http://so.56.com/video/%s/?',icons.v56]
	vo[4]=['六房间','http://6.cn/search.php?k=%s',icons.v6]
	vo[5]=['酷6','http://so.ku6.com/v/q%s&ie=utf-8&oe=utf-8',icons.ku6]
	vo[-6]=['google视频','http://www.google.com/search?q=%s&sourceid=opera&ie=utf-8&oe=utf-8&tbs=vid:1',icons.google]
	vo[-7]=['百度视频','http://video.baidu.com/v?word=%s&ie=utf-8&oe=utf-8',icons.baidu]
	vo[8]=['必应视频','http://cn.bing.com/videos/search?q=%s&pc=OPER&ie=utf-8&oe=utf-8',icons.bing]
	vo[-9]=['有道视频','http://video.youdao.com/search?q=%s',icons.youdao]

	//音乐搜索列表
	var mo=[];
	mo[0]=['百度mp3','http://mp3.baidu.com/m?f=ms&ct=134217728&word=%s&tn=cnopera&ie=utf-8&oe=utf-8',icons.baidu]
	mo[1]=['谷歌音乐','http://www.google.cn/music/search?q=%s&sourceid=opera',icons.google]
	mo[2]=['SoSo音乐','http://cgi.music.soso.com/fcgi-bin/m.q?w=%s&ie=utf-8&oe=utf-8',icons.soso]
	mo[3]=['有道音乐','http://mp3.youdao.com/search?q=%s',icons.youdao]
	mo[4]=['搜狗音乐','http://mp3.sogou.com/music.so?query=%s&ie=utf-8&oe=utf-8',icons.sogou]
	mo[5]=['一听音乐','http://so.1ting.com/all.do?q=%s',icons.m1ting]
	mo[6]=['Songtaste','http://www.songtaste.com/search.php?type=&keyword=%s',icons.songtaste,'gbk']
	mo[7]=['虾米音乐','http://www.xiami.com/search?key=%s',icons.xiami]
	mo[-8]=['gougou音乐','http://mp3.gougou.com/search?search=%s&ie=utf-8&oe=utf-8',icons.gougou]
	mo[9]=['好听音乐','http://seach.haoting.com/user/search2.asp?stype=Music&keyword=%s',icons.haoting,'gbk']
	mo[10]=['猫扑爱听','http://m.mop.com/search/?name=%s&ie=utf-8&oe=utf-8',icons.mop]
	mo[-11]=['雅虎音乐','http://search.cn.yahoo.com/s?p=%s&v=music&fr=opera2',icons.yahoo]
	mo[12]=['巨鲸音乐','http://www.top100.cn/search/index.aspx?keyword=%s',icons.top100]

	//图片搜索列表
	var po=[];
	po[0]=['百度图片','http://image.baidu.com/i?tn=baiduimage&ct=201326592&lm=-1&cl=2&word=%s&tn=cnopera&ie=utf-8&oe=utf-8',icons.baidu]
	po[1]=['谷歌图片','http://images.google.com/images?q=%s&sourceid=opera',icons.google]
	po[2]=['SoSo图片','http://image.soso.com/image.cgi?sc=img&w=%s&ie=utf-8&oe=utf-8',icons.soso]
	po[3]=['必应图片','http://cn.bing.com/images/search?q=%s&pc=OPER',icons.bing]
	po[4]=['有道图片','http://image.youdao.com/search?q=%s&keyfrom=web.top',icons.youdao]
	po[5]=['picsearch','http://cn.picsearch.com/index.cgi?&q=%s',icons.picsearch]
	po[6]=['搜狗图片','http://pic.sogou.com/pics?query=%s',icons.sogou,'gbk']
	po[-7]=['雅虎图片','http://one.cn.yahoo.com/s?p=%s&v=image&fr=opera2',icons.yahoo]
	po[8]=['flickr','http://www.flickr.com/search/?w=all&q=%s',icons.flickr]
	po[9]=['jpg4','http://img.jpg4.info/index.php?feed=%s']

	//下载列表
	var dlo=[];
	dlo[0]=['VeryCD','http://www.verycd.com/search/folders/%s',icons.verycd]
	dlo[1]=['狗狗','http://www.gougou.com/search?search=%s&id=utf-8',icons.gougou]
	dlo[2]=['绿盟','http://www.xdowns.com/tag.asp?keyword=%s',icons.xdowns,'gbk']
	dlo[3]=['绿色下载','http://search.greendown.cn/search.asp?action=s&sType=ResName&keyword=%s',icons.greendown,'gbk']
	dlo[4]=['射手字幕','http://shooter.cn/sub/?searchword=%s',icons.shooter]
	dlo[5]=['天空软件','http://www.skycn.com/search.php?ss_name=%s&sf=default',icons.skycn,'gbk']
	dlo[6]=['太平洋','http://ks.pconline.com.cn/download.jsp?q=%s',icons.pconline,'gbk']
	dlo[7]=['华军','http://search.newhua.com/search_list.php?searchname=%s',icons.newhua]
	dlo[-8]=['霏凡','http://search.crsky.com/search.asp?action=s&sType=ResName&keyword=%s',icons.crsky,'gbk']
	dlo[-9]=['完美天空','http://www.wmzhe.com/index.php?keyword=%s&controller=default&action=search',icons.wmzhe]

	//购物列表
	var sppo=[];
	sppo[0]=['淘宝','http://search.taobao.com/search?q=%s&ie=utf-8&oe=utf-8',icons.taobao]
	sppo[1]=['京东','http://search.360buy.com/Search?keyword=%s',icons.s360buy,'gbk']
	sppo[2]=['亚马逊','http://www.amazon.cn/s/ref=nb_ss?url=search-alias%3Daps&keywords=%s&searchKind=keyword',icons.amazon]
	sppo[3]=['淘日本','http://www.taojapan.com/search.htm?cid=&q=%s',icons.taojapan]
	sppo[4]=['当当','http://search.dangdang.com/search.php?catalog=&key=%s&SearchFromTop=1',icons.dangdang,'gbk']
	sppo[5]=['有啊','http://youa.baidu.com/search/s?search_domain=1&category=0&keyword=%s',icons.youa,'gbk']

	//翻译列表
	var trao=[];
	trao[0]=['海词','http://dict.cn/%s',icons.dict]
	trao[1]=['爱词霸','http://www.iciba.com/%s/',icons.iciba]
	trao[2]=['沪江','http://dict.hjenglish.com/app/w/%s',icons.hj]
	trao[3]=['有道词典','http://dict.youdao.com/search?&q=%s&tab=chn&keyfrom=dict.top',icons.youdao]
	trao[4]=['Google翻译','http://translate.google.cn/translate_t?&client=opera&q=%s',icons.google]

	//站内搜索,当前站点用 %i的变量表示.
	var ssao=[];
	ssao[0]=['Google','http://www.google.com/search?q=%s&sourceid=opera&ie=utf-8&oe=utf-8&sitesearch=%i',icons.google]
	ssao[1]=['百度','http://www.baidu.com/s?wd=%s&tn=cnopera&ie=utf-8&si=%i',icons.baidu]

	//控制列表的具体细节
	//list_details[排序(从小到大,小于0不显示相应列表)]=['显示在网页上的名字',列表名称(别改这个),列表数组(也别改)]
	var list_details=[];
	list_details[0]=['网页','web',wo];
	list_details[1]=['视频','video',vo];
	list_details[2]=['音乐','music',mo];
	list_details[3]=['图片','pic',po];
	list_details[4]=['下载','dl',dlo];
	list_details[5]=['购物','spp',sppo];
	list_details[6]=['翻译','tra',trao];
	list_details[7]=['站内','ssa',ssao];
...
///////////////////////////////////////////////
	function init(e){
		...
		function getPosition(node){
			return document.evaluate(node,document,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue;
		};
		...
		//转为GBK编码
		function getGbk(o_keyword){
			//怕转码函数出错..导致整个JS挂掉..用try捕捉并忽略错误..
			try{
				return document.characterSet.search(/gb.+/i)==-1? UrlEncode(o_keyword) : o_keyword;
			}catch(e){};
		};
	...
	var site_regexp,
		...
		if(prefs.E_advanced){
			var isites_x,
						...
						o_keyword;
			try{
				isites=cus_isites.concat(isites);
			}catch(e){};
			for(i=0,ii=isites.length;i<ii;i++){
				isites_x=isites[i];
				site_regexp=isites_x.site_regexp;
				//alert(site_regexp);
				site_enable=isites_x.enable;
				if(site_regexp.test(cur_href) && (site_enable || site_enable===undefined)){
					if(isites_x.loaded && !window.chrome && !(e && e.type=='load')){
						window.addEventListener('load',init,false);
						return;
					};
					//alert(site_regexp);
					getValueF=isites_x.keyword;
					try{
						o_keyword=getValueF();
					}catch(e){
						//alert('函数出错啦');
					...
					if(!o_keyword)break;
					if(isites_x.position instanceof Array){
						var position_x;
						for(j=0,jj=isites_x.position.length;j<jj;j++){
							position_x=isites_x.position[j];
							if(position_x instanceof Array){
								position =getPosition(position_x[0]);
								if(position){
									insert_method=position_x[1];
								};
							}else{
								position =getPosition(position_x);
							};
							if(position)break;
						};
					}else{
						position =getPosition(isites_x.position);
					};
					if(!position)break;
					if(insert_method===undefined)insert_method=isites_x.insert_method;
					slist=isites_x.slist;
					div_style=isites_x.div_style;
					ismatch=true;
					break;
				};
			};
		...
		if(!ismatch){
			if(!window.opera && !window.chrome){
				try{
					var S_toolbarM=GM_getValue('S_toolbarM');
					//alert(S_toolbarM);
					if(S_toolbarM!==undefined){
						prefs.E_TB=S_toolbarM;
					};
					GM_registerMenuCommand("启用/禁用工具栏模式",
						function(){
							var value=GM_getValue('S_toolbarM');
							if(value===undefined){
								GM_setValue('S_toolbarM',!prefs.E_TB);
							}else{
								GM_setValue('S_toolbarM',!value);
							};
							alert(GM_getValue('S_toolbarM')? '工具栏模式:启用' : '工具栏模式:禁用');
						});
				}catch(e){};
			};
			if(!prefs.E_TB)return;
			prefs.newpage=prefs.newpage2;
			position=getPosition('//body[1]/child::*[1]');
			if(!position)return;
			var fixedstyle='';
			if(prefs.fixed){
				fixedstyle+='\
			...
			div_style='\
				...
				-moz-box-shadow:0 5px 10px #ccc!important;\
				-webkit-box-shadow:0 5px 10px #ccc!important;\
		...
		global_style.textContent =g_sls;
		document.getElementsByTagName('head')[0].appendChild(global_style);

		//创建一个放置列表的div
		var sdiv=document.createElement('div');
		sdiv.setAttribute('id','slist');
		if(div_style)sdiv.setAttribute('style',div_style);//添加DIV样式进去

		//整理数组
		var domain=document.domain;
		function arraymod(array,main){
			function toahtml(){
				var href=' href="javascript:" ';
				var search=' search="'+array_x_eg.replace(/%i/i,domain)+'" ';
				var newpage=prefs.newpage? ' target="_blank" ': ' ';
				var encoding=gbk_e?' encoding="gbk" ':' ';
				var favicon=prefs.favicon? (array_x_img? '<img src="'+array_x_img+'" />' : ''):'';
				return '<a '+href+encoding+search+newpage+' >'+favicon+array_x_txt+'</a>';
			};
			var i,
						j,
						ii,
						...
						array_x_img,
						array_x_txt;
			for(i=j=0,ii=array.length;i<ii;i++){
				array_x=array[i];
				//alert(array_x);
				if(!array_x)continue;//空的项目删除
				array_x_eg=array_x[1];
				//alert(site_regexp);
				if(main && site_regexp.test(array_x_eg)){
					continue;//跳转到当前引擎的的删除
				};
				array_x_gbk=array_x[3];
				gbk_e=array_x_gbk && array_x_gbk.search(/gb.+/i)!=-1;
				array_x_img=array_x[2];
				array_x_txt=array_x[0];
				array_x=toahtml();
				//alert(array_x);
				fao[j]=array_x;
				j++;
			...
			if(main){
				sdiv.innerHTML='<span style="text-shadow:0 0 2px #ccc;">转到引擎: </span>'+ fao.join(prefs.separator);
			}else{
				return fao;
			};
		};
		...
		function ycqt(div){
			var i,
						ii,
						c_Nexdiv;
			for(i=0,ii=Nexdivs_l;i<ii;i++){
				c_Nexdiv=Nexdivs[i];
				if(c_Nexdiv!=div){
					c_Nexdiv.style.opacity='0';
					c_Nexdiv.style.height='0';
				};
			};
		};

		function other(array,text){
			var a=document.createElement('a');
			a.innerHTML=text;
			a.style.cssText='padding:0 0 0 10px;'
			a.href='javascript:';
			sdiv.appendChild(a);
			var div=document.createElement('div');
			div.className='Nexdiv';
			div.style.cssText='\
				opacity:0;\
				top:-99999px;\
				left=-99999px;\
			...
			var d_fa=div.getElementsByTagName('a')[0];
			//alert(d_fa.outerHTML);
			a.setAttribute('search',d_fa.getAttribute('search'));
			var d_fa_T=d_fa.getAttribute('target');
			if(d_fa_T){
				a.setAttribute('target',d_fa_T);
			};
			var d_fa_e=d_fa.getAttribute('encoding');
			if(d_fa_e){
				a.setAttribute('encoding',d_fa_e);
			};
			//alert(div.offsetHeight);
			//使用0延时.可以获取div的高度..这个要用来做YY动画.-_-!!;
			setTimeout(function(){
				var divH=window.getComputedStyle(div,'').height;
				if(Number(divH)==0)divH='auto';
				//alert('.'+divH);
				//获得高度后,隐藏它..
				div.style.height='0';
				...
				function X_clearTimeout(){
					clearTimeout(time_div);
					clearTimeout(time_a);
					clearTimeout(time_xs);
				};

				a.addEventListener('mouseover',function(e){
					ycqt(div);
					X_clearTimeout();
					div.style.top=this.getBoundingClientRect().top+window.scrollY+this.offsetHeight-2+'px';
					div.style.left=this.getBoundingClientRect().left+window.scrollX+'px';
					div.style.display='';
					setTimeout(function(){div_lashen()},0);
				},false);

				var time_xs,time_a,time_div;
				function dnone(){
					div.style.height=0;
					div.style.opacity=0;
					time_xs=setTimeout(function(){
						div.style.display='none';
					},300);
				};
		...
		var imgsrc='data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAGXRFWHRTb2Z0d2FyZQBBZG9iZSBJ  bWFnZVJlYWR5ccllPAAAAL1JREFUeNpi/P//PwMlgImBQjAMDGBBF8jOzj7+588fi79//zLAApiR  kZGBmZmZgYWF5cTUqVMt8RoA1GhRUlrGwMXFA2KDxUCav337wtDb022Brp4RPRqBthknJyWdiY6N  Y/jy7TtYjIeLk2Hp4kUMc+fNMwGqP4uiAWQAMoYC4+Tk5P9r160HYxAbJIZNPQuOsDk7d+5cE6Cf  z4A4N27ciAWJYVOIzQvIXGMofR2Iv8FcgNeA0aRMOgAIMADo32ORwCadLgAAAABJRU5ErkJggg==';
		...
		for(i in list_details){
			...
			//alert(i);
			c_list=list_details[i];
			//alert(c_list);
			if(!c_list)continue;
			if(ismatch && c_list[1]==slist){
				arraymod(c_list[2],true);
				continue;
			};
			if(i<0)continue;
			temp_array[j]=c_list;
		...
		if(!ismatch){
			...
			xinput.style.cssText='\
				...
				max-width:200px;\
			';
			xinput.id='N_searchvalue';
			xinput.title='请输入搜索内容';
			sdiv.appendChild(xinput);
			xinput.addEventListener('keydown',function(e){
				var SA=[
								'http://www.google.com/search?q=%s&sourceid=opera&ie=utf-8&oe=utf-8',
								'http://www.baidu.com/s?wd=%s&tn=cnopera&ie=utf-8'
				];
				if(e.keyCode==13){
					var value=this.value;
					if(!value)return;
					window.open(SA[prefs.D_open].replace(/%s/i,encodeURIComponent(value)))
				};
			},false);
		};

		if(!ismatch || (ismatch && !prefs.D_lists)){
			for(i=0,ii=list_details.length;i<ii;i++){
				c_list=list_details[i];
				if(c_list[2].length>0){
					other(arraymod(c_list[2]),c_list[0]+'<img src="'+imgsrc+'" />');
				};
			};
		};

		//最后将这个列表插入到网页中,方法1为插到元素前面,2为附加到元素里面
		insert_method=insert_method===undefined? '1' : insert_method;
		switch(String(insert_method)){
			case '1':{
				position.parentNode.insertBefore(sdiv,position);
			};break;
			case '2':{
				position.appendChild(sdiv);
			};break;
			default:{};break;
		};
		...
		function M_over(e){
			var target=e.target;
			var search=target.getAttribute('search');
			if(search){
				var input_value=ismatch? getValueF() : document.getElementById('N_searchvalue').value;
				//alert(input_value);
				var isgbk=target.getAttribute('encoding');
				if(isgbk && isgbk.toLowerCase().indexOf('gb')!=-1){
					var Gbkkey=getGbk(input_value);
					target.href=search.replace(/%s/i,Gbkkey);
				}else{
					var Utf8key=encodeURIComponent(input_value);
					target.href=search.replace(/%s/i,Utf8key);
				};
			};
		};

		//挂在mouseover监控输入框变化
		var Nexdivs=document.getElementsByClassName('Nexdiv');
		var Nexdivs_l=Nexdivs.length;

		var Nexdivs_x;
		for(i=0,ii=Nexdivs.length;i<ii;i++){
			Nexdivs_x=Nexdivs[i];
			Nexdivs_x.addEventListener('mouseover',M_over,false);
		};

		sdiv.addEventListener('mouseover',M_over,false);

		var Xevent=document.createEvent('HTMLEvents');
		Xevent.initEvent('search_EJ_loaded',true,false);
