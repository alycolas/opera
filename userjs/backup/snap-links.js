...
// @ujs:documentation http://bbs.operachina.com/viewtopic.php?f=41&t=34561
// ==/UserScript==
...
//..//
	...
	inSameTab=0, // open the first or last link in same tab: 1=first link in same tab , -1=last link in same tab , 0=open all in background
	reqCtrlKey=1, // run script only when alt key is pressed
...
//..//
	...
	events={
		mousedown:function(ev){
			...
			sl.clear(0,0,1);
			if(ev.which!==1 || ev.ctrlKey!=reqCtrlKey || selNode.call(ev.target,'ancestor-or-self::node()[contains(",input,textarea,img,button,select,a,object,embed,",concat(",",local-name(),",")) and (local-name()!="a" or @href)]'))return;
