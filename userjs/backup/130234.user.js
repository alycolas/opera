...
var cuCol = ['#0079B2','#E6F1F9','#004A6F','#FFFFFF']; //背景,字体,背景选中,字体选中
var rot = 0;
var cvScale = 1;
var tframeLoaded = 0;
var asetIrs = ['isea_setScstep','isea_setScmin','isea_setScmax','isea_setZtop'];
var defimgSet = ['0.3','0.2','2','10001']; //缩放变动值,最小缩放倍数,最大缩放倍数,查看框Z-Index
var tScaleStep; //缩放变动值
var tScaleMin; //最小缩放倍数
var tScaleMax; //最大缩放倍数
var zTop; //图片查看框z-index
var timgLarge; //默认查看新浪大图
var timglaBtn; //顶部新浪原图切换键
var timgLock ; //起始图片框相对固定
var tpageExtend; //页面扩展加载
var tpageLoaded = 1; //已经加载页数
var tkeyia = false;
var cuimgSet =[];
t_getSettings();
var imgList;
...
function t_getSettings(){
	cuimgSet[0] = GM_getValue('isea_scsetp',defimgSet[0]);
	cuimgSet[1] = GM_getValue('isea_scmin',defimgSet[1]);
	cuimgSet[2] = GM_getValue('isea_scmax',defimgSet[2]);
	cuimgSet[3] = GM_getValue('isea_ztop',defimgSet[3]);
	tScaleStep = parseFloat(cuimgSet[0]);
	tScaleMin = parseFloat(cuimgSet[1]);
	tScaleMax = parseInt(cuimgSet[2]);
	zTop = parseInt(cuimgSet[3]);
	timgLarge = GM_getValue('isea_imglarge','false');
	timglaBtn = GM_getValue('isea_imglabtn', 'checked');
	timgLock = GM_getValue('isea_imglock','checked');
	tpageExtend = GM_getValue('isea_pageext','false');
}
...
t_set_html='<div id="isea_setdiv"class="t_showframe"><div class="t_setdiv"align="center"><div style="padding-top:5px;padding-bottom:5px;"><b>iSea 4 Jandan 设置</b></div><table class="t_set_tb"border="0"cellspacing="0"cellpadding="0"width="90%"><thead><tr><td colspan="2"align="left"><b>图片查看</b></td></tr></thead><tr><td style="width:15%;">新浪微博图</td><td style="width:85%;"><input type="radio"class="t_rtjdchk"id="isea_imgLarge"name="simgsize"/>查看原图大小　<input type="radio"class="t_rtjdchk"id="isea_imgMid"name="simgsize"/>查看中等大小　　　顶部显示新浪原图切换键: <input type="checkbox"class="t_rtjdchk"id="isea_setimgTitle"/></td></tr><tr><td>图片缩放</td><td>缩放变动值: <input id="isea_setScstep"class="t_setfloat"type="text"style="width:30px;"/> <span id="isea_irs1"class="t_setbtn t_settrs">重置</span>　最小缩放倍数: <input id="isea_setScmin"class="t_setfloat"type="text"style="width:30px;"/> <span id="isea_irs2"class="t_setbtn t_settrs">重置</span>　最大缩放倍数: <input id="isea_setScmax"class="t_setnum"type="text"style="width:30px;"/> <span id="isea_irs3"class="t_setbtn t_settrs">重置</span></td></tr><tr><td>查看框</td><td>相对固定: <input type="checkbox"class="t_rtjdchk"id="isea_setimgLock"/>　Z-Index: <input id="isea_setZtop"class="t_setnum"type="text"style="width:45px;"/> <span id="isea_irs4"class="t_setbtn t_settrs">重置</span>　　加载前页图片: <input type="checkbox" class="t_rtjdchk" id="isea_setpageext" /></td></tr><tfoot><tr><td colspan="2"style="line-height:15px">说明：【新浪微博图】 对新浪微博图选择默认显示的尺寸<br/>　　　　　　　　　 　在查看中等大小新浪微博图时顶部是否显示原图切换键，也可双击图片切换<br/>　　　【图片缩放】 图片缩放的步进变动值与可缩放的最小、最大倍数<br/>　　　【查看框】　 查看框被拖动前是否相对固定不动<br/>　　　　　　　　　 当查看框被网页内其他元素遮挡时可通过增大Z-Index值解决<br />　　　【快捷键】　 左右方向键切换图片,Ctrl键定位图片,小键盘0键切换原图,Shift键关闭看图框<br />注意：请在图片显示后再按左右键切换,勿快速重复按键,如按键过快引起问题,多按F5键刷新页面</td></tr></tfoot></table><div class="t_set_ft"style="width:95%"><table cellspacing="0"cellpadding="0"style="width: 100%;"><tr><td style="width: 50%;text-align:left;"><a href="http://t.163.com/rok"target="_blank"></a></td><td style="width: 50%;text-align:right" class="t_setftbtn"><span id="isea_setsave"class="t_setbtn t_savbtn">保存</span>　<span id="isea_setcancel"class="t_setbtn">取消</span>　<span id="isea_help"class="t_setbtn">帮助</span>　<span id="isea_update"class="t_setbtn">检查更新</span></td></tr></table></div></div></div>';

is_seaMore();

function is_seaMore(){
    $('ol.commentlist li p img').each(function(){
        $(this).css('cursor','pointer');
        $(this).click(function(event){
	        imgList = $('ol.commentlist li p img');
	        curImg = imgList.index($(this));
	        t_showImgbox($(imgList.get(curImg)).attr('src'),t_getNavimgInfo(false),false,false);
        });
    });
    
    $('div#sidebar div.hotcomment div.acv_comment img').each(function(){
        $(this).css('cursor','pointer');
        $(this).click(function(event){
            imgList = $('div#sidebar div.hotcomment div.acv_comment img');
            curImg = imgList.index($(this));
            t_showImgbox($(imgList.get(curImg)).attr('src'),t_getNavimgInfo(true),false,true);
        });
    });
}

function t_showImgbox(sImg,sInfo,isNav,isHot){
    isCancelimg = false;
    tframeLoaded = 0;
    var sleft = (document.documentElement.clientWidth - 550 ) /2;
    $.blockUI({
        message: '<div id="isea_imgloading" class="t_showframe t_txtshow t_loadshow"><div id="isea_loadtxt"><img src="'+ loadpic +'" />　载入中,若图片太大需时读入,请稍候.若长时间无响应则可能图片已被删除<br>双击中间定位查看,点击左、右两边切换上、下一张图或点击背景返回</div><div class="t_imgprev"></div><div class="t_imgnext"></div></div>' , 
        css: {
                border: '',
                width: '520px',
                left: sleft+'px',
    ...
    if(isHot){
    	var curItem = $(imgList.get(curImg)).parent().prev();
    }else{
		var curItem = $(imgList.get(curImg)).parent().parent();
	}
    if(isNav){
        setTimeout(function(){
            t_scrollToLoading(curItem);
        },500);
    }else{
        t_scrollToLoading(curItem);
    }
	if(timgLarge=='checked'){
		if(sImg.indexOf('.sinaimg.cn/bmiddle/')>-1){
			sImg = sImg.replace('/bmiddle/','/large/');
		}
	}
    imgReady(sImg,function(sW,sH){
        if(isNav){
            setTimeout(function(){
                t_showImg(sImg,sW,sH,sInfo,isHot);
            },500);
        }else{
            t_showImg(sImg,sW,sH,sInfo,isHot);
        }
    ...
    t_setNaviAction(isHot);
}

function t_scrollToLoading(curItem){
	$('#isea_loadtxt').dblclick(function(){
		$('html,body').animate({scrollTop: curItem.offset().top}, 500);	
	});
}

function t_showImg(sImg,sW,sH,sInfo,isHot){
    if(tframeLoaded==1){
        return;
    }
    ...
    var stop = (document.documentElement.clientHeight - oriImgH - 100) /2;
    if(stop<0)  stop = 0;
    if(timgLock!='checked'){
    ...
    if(sleft<0)sleft = 0;
    $.blockUI({ 
        message: '<div id="isea_imgframe"><div class="t_imgtitle"><div><span id="isea_imgpage" style="float:left;"><ul class="t_customCss"><li><a class="t_imgbtn">页面</a></li></ul></span><span class="t_imglocate t_imgurl"><b><a>定位</a></b></span> | <span class="t_imgurl"><b><a target=_blank href='+sImg+'>另存</a></b></span><span id="isea_imgla"style="float:right;"><ul class="t_customCss"><li><a class="t_imgbtn">原图</a></li></ul></span></div></div><div id="isea_imgarea"><canvas id="isea_canvas"style="display:none;"></canvas><img id="isea_showimg"src="'+ sImg + '"title="'+sHint+'"/><div class="t_imgprev"><span></span></div><div class="t_imgnext"><span></span></div><span id="isea_turnspan"><a id="turnLsign"class="t_turnsign"><img src="'+ tleftimg +'"title="向左转"/></a>&nbsp;<a id="turnRsign"class="t_turnsign"><img src="'+ trightimg +'"title="向右转"/></a></span><span id="isea_zoomspan"><a id="tZoomori"class="t_turnsign"><img src="'+ tzoriimg +'"title="还原"/></a>&nbsp;<a id="tZoomout"class="t_turnsign"><img src="'+ tzoutimg +'"title="缩小"/></a>&nbsp;<a id="tZoomin"class="t_turnsign"><img src="'+ tzinimg +'"title="放大"/></a></span></div><div class="t_imginfo">'+ sInfo + '<div id="isea_imgurl"><span class="t_imglocate t_imgurl"><b><a>定位</a></b></span> | <span class="t_imgurl"><b><a target=_blank href='+sImg+'>另存</a></b></span></div><span style="float:right;"><ul class="t_customCss"><li><a id="isea_setting"class="t_imgbtn">设置</a></li></ul></span></div></div>',
        css: {
            top: stop + 'px', 
            left: sleft + 'px',
            width: sW+20+'px',
    ...
    $('#isea_setting').bind('click',function(){
    	t_closeImg();
    	t_Settings();
    });
    if(sImg.indexOf('.sinaimg.cn/bmiddle/')>-1){
    	if(timglaBtn=='checked'){
    		$('#isea_imgframe #isea_imgla').show();
    	}
    	$('#isea_imgarea').dblclick(function(){
    		$('#isea_imgla').click();
    	});
    }else{
    	$('#isea_imgarea').dblclick(t_toogleImgSize);
	}
    ...
    $('#isea_imgframe').hover(function(){
            $('#isea_turnspan').css('display','block');
            $('#isea_zoomspan').css('display','block');
        },
        function(){
            $('#isea_turnspan').css('display','none');
            $('#isea_zoomspan').css('display','none');
        }
    )
    
    $('.blockPage').css('background-color','transparent');
    $('.blockPage .t_imginfo a').not('#isea_imgrt').attr('target','_blank');
    ...
    if(sImg.indexOf('/bmiddle/')>-1){
    	if(timglaBtn=='checked'){
		    ...
		    $('#isea_imgla').click(function(){
		    	t_closeImg();
				t_showImgbox(sImg.replace('/bmiddle/','/large/'),sInfo,true,isHot);
		    });
    	}
    ...
    if($(imgList.get(curImg)).parents('div.t_pagebox').length>0){
		var loadPage = $(imgList.get(curImg)).parent().parent().parent().attr('id').substr(9);
		$('#isea_imgpage a').text('-' + loadPage + '-');
    	$('#isea_imgpage').show().click(function(){
	    	var curUrl = location.href;
	    	var nextUrl = '';
	    	if(curUrl.indexOf('#comments')>-1){
	    		var tmps = curUrl.indexOf('page-');
	    		nextUrl = curUrl.substring(0,tmps+5) + loadPage  + '#comments';
	    	}else{
	    		nextUrl = curUrl + '/page-'+loadPage+'#comments';
	    	}
    ...
    if(isHot){
        voteId = $(imgList.get(curImg)).parent().next().attr('id');
    }else{
        voteId = $(imgList.get(curImg)).parent().parent().find('div.vote').attr('id');
    }
    $('#'+voteId).insertAfter($('#isea_imgurl'));
    t_setNaviAction(isHot);
    tframeLoaded = 1;
...
function t_setNaviAction(isHot){
    if(curImg>0){
        $('div.t_imgprev').unbind('click').bind('click',function(){
        	if(tframeLoaded==1){
	            t_closeImg();
	        }else{
	       		$.unblockUI();
            ...
            curImg--;
            t_showImgbox($(imgList.get(curImg)).attr('src'),t_getNavimgInfo(isHot),true,isHot);
        }).mouseenter(function(){
    	...
    	if(!isHot){
	    	var aNavi = $('#comments p.cp-pagenavi a.next-comment-page:first');
	    	if(aNavi.length>0)
	    	{
	    		$('div.t_imgprev').css('cursor','sw-resize');
	    		$('div.t_imgprev').unbind('click').bind('click',function(){
	    			$.unblockUI();
	    			t_Hint(1);
	    			location.href=aNavi.attr('href');
	    		});
	    	}
	    }
    }
    if(curImg<imgList.length-1){
        $('div.t_imgnext').unbind('click').bind('click',function(e){
        	if(tframeLoaded==1){
	            t_closeImg();
	        }else{
	       		$.unblockUI();
    	...
    	if(!isHot){
	    	var aNavi = $('#comments p.cp-pagenavi a.previous-comment-page:first');
	    	if(aNavi.length>0)
	    	{
	    		$('div.t_imgnext').css('cursor','se-resize');
	    		$('div.t_imgnext').unbind('click').bind('click',function(){
		    		t_closeImg();
	    			if(tpageExtend!='checked'){
	    				t_Hint(2);
	    				location.href=aNavi.attr('href');
	    			}else{
				    	var curPage = $('span.current-comment-page');
				    	if(curPage.length>0){
				    		curPage = parseInt(curPage.eq(0).text().replace(/\[|\]/g,''));
				    	}else{
				    		return;
				    	}
				    	var curUrl = location.href;
				    	var nextUrl = '';
				    	var loadPage = curPage-tpageLoaded;
				    	if(loadPage <1){
				    		t_Hint(4);
				    		return;
				    	}
					    ...
					    GM_xmlhttpRequest({
							...
							onload: function(result) {
								var chkhtml = result.responseText.replace(/(^\s*)|(\s*$)|([\r])|([\n])/g, '');
								chkhtml = chkhtml.replace(/([\s\S]*)<ol class="commentlist" style="list-style-type: none;">/,'').replace(/<\/ol><div class="comments"><p class="cp-pagenavi">([\s\S]*)/,'').replace('<li style="padding:15px 32px;"><script type="text/javascript">/*336*280*/ var cpro_id = \'u529095\';</script><script src="http://cpro.baidu.com/cpro/ui/c.js" type="text/javascript"></script></li>','');
								$('<div class="t_pagebox" id="isea_page'+ loadPage +'"><li class="t_pagecount"><div>-&nbsp;'+loadPage+'&nbsp;-</div></li></div>').appendTo('ol.commentlist');
								$(chkhtml).appendTo('ol.commentlist div#isea_page'+loadPage);
								setTimeout(function(){
									is_seaMore();
									t_HintOut();
							        imgList = $('ol.commentlist li p img');
...
function t_getNavimgInfo(isHot){
    var curItem;
    var imgInfo;
    if(isHot){
        curItem = $(imgList.get(curImg)).parent();
        imgInfo = curItem.prev().text();
        imgInfo = '<b>' + imgInfo.substring(0,imgInfo.indexOf('@')-1) + '</b> ' + imgInfo.substring(imgInfo.indexOf('@')) + '<br>';
        imgInfo = imgInfo + curItem.text();
    }else{
        curItem = $(imgList.get(curImg)).parent().parent();         
        imgInfo = '<b>' + curItem.find('b').text() + '</b> ' + curItem.find('span.time').text()+'<br>';
        imgInfo = imgInfo + curItem.find('p').text() 
    }
    return imgInfo;
}

function t_closeImg(){
    $('#'+voteId).insertAfter($(imgList.get(curImg)).parent());
    $.unblockUI();    
}

function t_imgFrametune(){
    var cvW = $('#isea_canvas').width();
    $('.blockPage').css('width',cvW+20+'px');
    
    var cvH = $('#isea_canvas').height();
...
function t_imgZoom(imode){
    ...
    rotate(document.getElementById("isea_canvas"),document.getElementById("isea_showimg"));
    t_imgFrametune();    
}
...
function t_toogleImgSize(){
    ...
    var oriImgW = $('#isea_showimg').width();
    var oriImgH = $('#isea_showimg').height();
    if(rot!=0 && rot!=180){
        var tmp = oriImgH;
        oriImgH = oriImgW;
        oriImgW = tmp;
    ...
    var cvH = $('#isea_canvas').height();
    var cvShow = 1;
    if($('#isea_canvas').css('display')=='none'){
        cvW = oriImgW;
        cvH = oriImgH;
        cvShow = 0;        
    };
...
function t_Hintpos(){
    var iW = $('#isea_hint').width() +20;
    var iH = $('#isea_hint').height() +10;
    var ileft = (document.documentElement.clientWidth - iW)/2;
    $('#isea_hint').css('top',(document.documentElement.clientHeight - iH -200)/2+'px').css('left',ileft+'px');
}

function t_Hint(iMode){
    var hint = '';
    switch(iMode){
        case 1:
            hint = '正在打开下一页，请稍候';
    ...
    if($('#isea_hint').length==0){
        $('<div id="isea_hint" class="t_hint_n">'+ hint +'</div>').appendTo('body').hide();
        t_Hintpos();        
    }
    if(iMode == 4){
	    $('#isea_hint').fadeIn(200,function(){
	        setTimeout(t_HintOut,2500);
        });
    }else{
    	$('#isea_hint').show();
	}
...
function t_HintOut(){
    $('#isea_hint').fadeOut(200,function(){
        $('#isea_hint').remove();
    });
}
...
function t_Settings(){
    ...
    $('#isea_setdiv input:radio').checkbox({cls:'t_jradbox',empty:cbemptyimg});
    $('#isea_setdiv input:checkbox').checkbox({cls:'t_jchkbox',empty:cbemptyimg});
    $('.blockPage').css('z-index',zTop);
    $('.t_set_ft a:first').text('iSea 4 Jandan v'+ p_version + ' - forumz @ ' + p_rdate);
    $('.blockPage').css('background-color','transparent');
    $('#isea_setcancel').click($.unblockUI);
    $('.blockOverlay').click(function(){
        $.unblockUI();
    });
    if(timgLarge == 'checked'){
    	$('#isea_imgLarge').attr('checked','checked');
    }else{
    	$('#isea_imgMid').attr('checked','checked');
    }
    if(timglaBtn == 'checked'){
    	$('#isea_setimgTitle').attr('checked','checked');
    }
    if(timgLock == 'checked'){
    	$('#isea_setimgLock').attr('checked','checked');
    }
    if(tpageExtend == 'checked'){
    	$('#isea_setpageext').attr('checked','checked');
    }
    ...
    $('#isea_setsave').click(function(){
    	var ival;
    	if($('#isea_imgLarge').attr('checked')=='checked'){
    		ival = 'checked';
    	}else{
    		ival = false;
    	}
    	GM_setValue('isea_imglarge',ival);
    	if($('#isea_setimgTitle').attr('checked')=='checked'){
    		ival ='checked';
    	}else{
    		ival = false;
    	}
    	GM_setValue('isea_imglabtn',ival);
    	if($('#isea_setimgLock').attr('checked')=='checked'){
    		ival = 'checked';
    	}else{
    		ival = false;
    	}
    	GM_setValue('isea_imglock',ival);
    	if($('#isea_setpageext').attr('checked')=='checked'){
    		ival = 'checked';
    	}else{
    		ival = false;
    	}
    	GM_setValue('isea_pageext',ival);
		if(parseInt($('#'+asetIrs[3]).val())<=1000){$('#'+asetIrs[3]).val('10001');}
    	GM_setValue('isea_scsetp',$('#'+asetIrs[0]).val());
    	GM_setValue('isea_scmin',$('#'+asetIrs[1]).val());
    	GM_setValue('isea_scmax',$('#'+asetIrs[2]).val());
    	GM_setValue('isea_ztop',$('#'+asetIrs[3]).val());
		t_getSettings();
    	$.unblockUI();
    });
    $('#isea_update').click(function(){
        update(true);
    });
    $('#isea_help').click(function(){
...
{
  autoUpdateFromUserscriptsDotOrg({
    url: 'http://userscripts.org/scripts/source/130234.user.js',
    version: p_version,
    forceUpdate: forceUpdate,
    web: 'http://userscripts.org/scripts/show/130234'
  });
}
...
if(GM_registerMenuCommand){
	GM_registerMenuCommand('从 userscript.org 更新 iSea 4 Jandan',function(){update(true)});
}